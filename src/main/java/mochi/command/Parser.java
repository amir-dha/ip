...
public class Parser {
    ...
    public static Command parse(String input) throws MochiException {
        ...
        switch (pts[0]) {
            ...
            case "mark":
                if (pts.length < 2) throw new MochiException(" Bro, which task you want me to mark??");
                try {
            ...
            case "unmark":
                if (pts.length < 2) throw new MochiException(" Great, now you want me to read your mind to unmark a task.");
                try {
                    int taskInd = Integer.parseInt(pts[1]);
            ...
            case "todo":
                if (pts.length < 2) throw new MochiException(" Aigoo, you need description to make todo sis!!");
                return new AddCommand(new Todo(pts[1]));
            case "deadline":
                String[] deadlinePts = pts[1].split(" /by ");
                if (deadlinePts.length < 2) throw new MochiException(" Yo, deadlines need description, and when to do '/by'.");
                return new AddCommand(new Deadline(deadlinePts[0], deadlinePts[1]));
            case "event":
                String[] eventPts = pts[1].split("\\s*/from\\s*|\\s*/to\\s*");
                if (eventPts.length < 3) throw new MochiException(" Babes, events need a description, a '/from' time and a '/to' time.");
                return new AddCommand(new Event(eventPts[0], eventPts[1], eventPts[2]));
            case "delete":
                if (pts.length < 2) throw new MochiException(" Oi you need tell me which number to delete??");
                try {
                    int taskInd = Integer.parseInt(pts[1]);
